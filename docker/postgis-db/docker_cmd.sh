...
until docker run --rm --link $cont_name:pg mappy/postgis-data pg_isready -U redpig -h pg; do sleep 5; done

docker run -it --link $cont_name:postgres --rm postgres \
	    sh -c 'exec psql -h "$POSTGRES_PORT_5432_TCP_ADDR" -p "$POSTGRES_PORT_5432_TCP_PORT" -U oryx -d gdelt'
